...
function EventCard({ event }: { event: typeof events[0] }) {
  return (
    <Link
      ...
      style={{ display: 'block', position: 'relative', overflow: 'hidden', borderRadius: 12, border: `1px solid ${C.bd}`, background: C.bg2, textDecoration: 'none', transition: 'transform 300ms ease, border-color 300ms' }}
    >
      {event.hot && (
      ...
      <div style={{ width: '100%', paddingTop: '58%', position: 'relative', overflow: 'hidden', background: C.bg3 }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${event.image})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'brightness(0.72) saturate(0.82)' }} />
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4, scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}>
          <Link href={`/events/${trending[0]?.slug}`} style={{ position: 'relative', flexShrink: 0, width: 'min(75vw, 480px)', height: 'min(50vw, 300px)', borderRadius: 12, overflow: 'hidden', border: `1px solid ${C.bd}`, textDecoration: 'none', display: 'block' }}>
          ...
          {trending.slice(1).map(ev => (
            <Link key={ev.id} href={`/events/${ev.slug}`} style={{ position: 'relative', flexShrink: 0, width: 'min(60vw, 320px)', height: 'min(40vw, 220px)', borderRadius: 12, overflow: 'hidden', border: `1px solid ${C.bd}`, textDecoration: 'none', display: 'block' }}>
        ...
        <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4, scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}>
          {weekend.map(ev => (
            <Link key={ev.id} href={`/events/${ev.slug}`} style={{ flexShrink: 0, width: 148, borderRadius: 10, overflow: 'hidden', border: `1px solid ${C.bd}`, background: C.bg2, textDecoration: 'none', display: 'block' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
          {MOMENTS.map((m, i) => (
            <div key={i} style={{ position: 'relative', overflow: 'hidden', borderRadius: 10, border: `1px solid ${C.bd}`, aspectRatio: i === 0 ? '16/9' : '9/16', gridColumn: i === 0 ? 'span 2' : 'span 1' }}>
        ...
        <div style={{ marginTop: 56, borderRadius: 14, overflow: 'hidden', border: `1px solid ${C.bd}`, background: C.bg2 }}>
          <div style={{ padding: '40px 28px 36px', borderBottom: `1px solid ${C.bd}` }}>
            ...
            <Link href="#" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: C.gold, color: C.onAccent, fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, letterSpacing: '2px', textTransform: 'uppercase', padding: '14px 24px', borderRadius: 6, textDecoration: 'none', fontWeight: 700 }}>
        ...
        <div style={{ margin: '48px 0 8px', padding: '36px 28px', borderRadius: 14, border: `1px solid ${C.bd}`, background: C.bg2, position: 'relative', overflow: 'hidden' }}>
          ...
          <Link href="#" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: C.gold, color: C.onAccent, fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, letterSpacing: '2px', textTransform: 'uppercase', padding: '13px 24px', borderRadius: 6, textDecoration: 'none', fontWeight: 700 }}>
