...
export default function FromLinkPage() {
  ...
  async function publish(status: 'draft' | 'published' | 'pending_review') {
    ...
    try {
      ...
      router.push(status === 'pending_review' ? '/organizer/review' : '/organizer/dashboard');
    } catch (e: any) {
        ...
        {title !== '' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => publish('pending_review')} disabled={saving} style={{ flex: 1, background: C.goldDim, color: C.gold, border: `1px solid ${C.goldBd}`, padding: 14, borderRadius: 8, fontFamily: 'var(--font-dm-mono), monospace', fontSize: 8, letterSpacing: '1.5px', textTransform: 'uppercase', cursor: 'pointer' }}>
                Save for review
              </button>
