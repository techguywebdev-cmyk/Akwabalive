...
import LineupPicker, { lineupPayload, type LineupValue } from '@/components/organizer/LineupPicker';
...
const C = {
  ...
  bd: 'rgba(245,236,215,0.07)', red: '#CE1126', greenL: '#4ade80',
...
export default function FromLinkPage() {
  ...
  const [lineupVal, setLineupVal] = useState<LineupValue>({ artistSlugs: [], extraNames: '' });
  ...
  async function publish(status: 'draft' | 'published') {
    ...
    try {
      ...
      const { error: evErr } = await supabase.from('events').insert({
        ...
        lat: lat ?? null,
        lng: lng ?? null,
        artist_slugs: bill.artist_slugs.length ? bill.artist_slugs : null,
        lineup: bill.lineup.length ? bill.lineup : null,
      });
          ...
          <Field label="Post or ticket URL">
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={parse} disabled={loading || !url}
                style={{ background: C.gold, color: '#0D0B08', border: 'none', padding: '0 18px', borderRadius: 8, fontFamily: 'var(--font-dm-mono), monospace', fontSize: 8, letterSpacing: '1.5px', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
        ...
        {title !== '' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => publish('published')} disabled={saving} style={{ flex: 2, background: C.gold, color: '#0D0B08', border: 'none', padding: 14, borderRadius: 8, fontFamily: 'var(--font-dm-mono), monospace', fontSize: 9, letterSpacing: '2px', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
