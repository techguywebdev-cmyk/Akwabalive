      ...
      <div style={{ paddingTop: 62, maxWidth: 680, margin: '0 auto', padding: '80px 20px 80px' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginTop: 20 }}>
            {(['Event Details', 'Ticket Tiers', 'Review & Publish'] as const).map((label, i) => (
              <div key={label} style={{ display: 'flex', alignItems: 'center', flex: i < 2 ? 1 : 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: step > i + 1 ? C.green : step === i + 1 ? C.gold : C.bg3, border: `1px solid ${step > i + 1 ? C.greenBd : step === i + 1 ? C.gold : C.bd}`, fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 10, fontWeight: 600, color: step >= i + 1 ? '#000' : C.c3, flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ background: C.bg2, border: `1px solid ${C.bd}`, borderRadius: 14, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <Input label="Event Title *">
                <input type="text" value={title} onChange={e => setTitle(e.target.value)}
                  placeholder="e.g. Rapperholic 2025" style={inputStyle} />
              </Input>

              <Input label="Description">
                <textarea value={description} onChange={e => setDescription(e.target.value)}
                  placeholder="Tell people what to expect. This is your pitch."
                  rows={4}
              ...
              <Input label="Venue *">
                <input type="text" value={venue} onChange={e => setVenue(e.target.value)}
                  placeholder="e.g. Accra Sports Stadium" style={inputStyle} />
              </Input>
            </div>
            ...
            <div style={{ background: C.bg2, border: `1px solid ${C.bd}`, borderRadius: 14, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <p style={{ fontFamily: 'var(--font-inter,sans-serif)', fontSize: 12, color: C.c3, lineHeight: 1.6 }}>
                Paste any image URL. We recommend Unsplash for high quality images. Full image upload coming soon.
              </p>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {tiers.length < 4 && (
              <button onClick={addTier}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, border: `1px dashed ${C.bd}`, background: 'transparent', color: C.c3, padding: '14px', borderRadius: 10, cursor: 'pointer', fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, letterSpacing: '2px', textTransform: 'uppercase', transition: 'all 180ms', width: '100%' }}>
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: C.bg2, border: `1px solid ${C.bd}`, borderRadius: 14, overflow: 'hidden' }}>
              {imageUrl && (
                <div style={{ height: 200, backgroundImage: `url(${imageUrl})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'brightness(0.6)' }} />
              )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {tiers.map((t, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: C.bg3, borderRadius: 8, border: `1px solid ${C.bd}` }}>
                        ...
                        <span style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 16, color: parseFloat(t.price) === 0 ? '#4ade80' : C.cream, letterSpacing: '-0.5px' }}>
            ...
            {success && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(45,106,79,0.15)', border: '1px solid rgba(45,106,79,0.35)', borderRadius: 8, padding: '12px 14px' }}>
                <CheckCircle size={14} style={{ color: '#4ade80', flexShrink: 0 }} />
                <p style={{ fontFamily: 'var(--font-inter,sans-serif)', fontSize: 12, color: '#4ade80' }}>Event created! Redirecting to dashboard…</p>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => handlePublish('published')} disabled={saving || success}
                style={{ flex: 2, background: saving || success ? '#2D6A4F' : C.gold, color: '#0D0B08', border: 'none', padding: 14, borderRadius: 8, fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 9, letterSpacing: '2.5px', textTransform: 'uppercase', fontWeight: 700, cursor: (saving || success) ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'background 250ms' }}>
