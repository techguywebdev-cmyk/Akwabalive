...
export default function MomentsPage() {
  ...
  return (
    <div
      style={{ background: '#000', height: '100svh', width: '100vw', overflow: 'hidden', position: 'fixed', top: 0, left: 0, display: 'flex', flexDirection: 'column', userSelect: 'none' }}
      onTouchStart={handleTouchStart}
      onTouchEnd={handleTouchEnd}
    >
      {/* FIXED TOP BAR */}
      <div style={{ height: 56, flexShrink: 0, position: 'relative', zIndex: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 18px', background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {moment?.is_featured && (
            <span style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 7, letterSpacing: '1.5px', textTransform: 'uppercase', padding: '4px 10px', borderRadius: 20, background: 'rgba(200,146,42,0.18)', color: '#C8922A', border: '1px solid rgba(200,146,42,0.3)' }}>Featured</span>
          )}
      ...
      {/* VIDEO AREA — fills exactly remaining height */}
      ...
      {moment && (
        <iframe
          ...
          style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 'calc((100svh - 56px) * 16 / 9)', height: '100%', minWidth: '100%', border: 'none', zIndex: 1 }}
      ...
      {/* Gradient overlay */}
      <div style={{ position: 'absolute', inset: 0, zIndex: 2, background: 'linear-gradient(to top, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.15) 45%, transparent 70%)', pointerEvents: 'none' }} />
      {/* Block YouTube bottom bar */}
