...
export default function MomentsPage() {
  ...
  // Reset video state on moment change
  useEffect(() => { setShowVideo(false); }, [activeIdx]);
  ...
  return (
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#000', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#000' }}>
        ...
        {/* YouTube iframe — only rendered after tap to avoid gap */}
        {moment && showVideo && (
          <iframe key={moment.id}
            src={`https://www.youtube.com/embed/${moment.youtube_id}?autoplay=1&mute=0&controls=1&rel=0&modestbranding=1&playsinline=1&enablejsapi=1&iv_load_policy=3&fs=0`}
            style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', height: '100%', width: 'calc(100% * 16 / 9 * 1.05)', minWidth: '100%', border: 'none', zIndex: 1 }}
            allow="autoplay; fullscreen" allowFullScreen
          />
        )}

        {/* Thumbnail poster — covers gap until video loads, tap to play */}
        {moment && !showVideo && (
          <div onClick={() => setShowVideo(true)}
            style={{ position: 'absolute', inset: 0, zIndex: 5, cursor: 'pointer', background: '#000' }}>
            <img
              src={`https://img.youtube.com/vi/${moment.youtube_id}/maxresdefault.jpg`}
              alt={moment.title}
              style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
              onError={e => { (e.target as HTMLImageElement).src = `https://img.youtube.com/vi/${moment.youtube_id}/hqdefault.jpg`; }}
            />
            {/* Dark overlay on thumbnail */}
            <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.38)' }} />
            {/* Play button */}
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 68, height: 68, borderRadius: '50%', background: 'rgba(200,146,42,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="#000"><polygon points="5,3 19,12 5,21" /></svg>
            </div>
          </div>
        )}

        {/* Block YouTube bottom controls when playing */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 88, background: 'rgba(0,0,0,0.98)', zIndex: 4, pointerEvents: 'none' }} />
