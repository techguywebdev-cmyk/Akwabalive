...
import { MessageCircle, X, Send, Gift, Heart, Share2, Search, Ticket, ArrowLeft } from 'lucide-react';
...
function CustomPlayer({ youtubeId, title }: { youtubeId: string; title: string }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#000', zIndex: 1 }}>
      ...
      {/* No play button — tap anywhere on video to play/pause */}
...
export default function MomentsPage() {
  ...
  function handleTouchEnd(e: React.TouchEvent) {
    ...
    // Allow swipe in both directions with 35px threshold
    if (Math.abs(dy) < 35) return;
    if (dy > 0) {
      // Swipe up — next moment
      if (activeIdx < moments.length - 1) { setActiveIdx(i => i + 1); setShowComments(false); }
    } else {
      // Swipe down — previous moment (dy is negative)
      if (activeIdx > 0) { setActiveIdx(i => i - 1); setShowComments(false); }
    }
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#000', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#000' }}>
        ...
        <div style={{ position: 'absolute', right: 12, bottom: 160, zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
          ...
          <GlassBtn icon={<Search size={20} style={{ color: C.cream }} />} label="Search" onClick={() => {}} />
