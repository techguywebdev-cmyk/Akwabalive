...
export default function MomentsClient() {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (showComments || lightbox || showMenu) return;
  ...
  }, [goNext, goPrev, showComments, lightbox, showMenu, bumpChrome]);
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', inset: 0, background: '#000' }}>
        ...
        <div
          style={{
            ...
            zIndex: 3,
            background:
              'linear-gradient(to top, rgba(0,0,0,0.4) 0%, rgba(0,0,0,0.08) 28%, transparent 48%)',
            pointerEvents: 'none',
          }}
        />
      </div>
      ...
      {heartBurst && (
        ...
        >
          <Heart size={72} fill="#e53935" color="#e53935" style={{ filter: 'drop-shadow(0 8px 24px rgba(0,0,0,0.5))' }} />
        </div>
      )}
      ...
      {showMenu && (
        <MenuSheet onClose={() => setShowMenu(false)} username={profile?.username} />
      )}
