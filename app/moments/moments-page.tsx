  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#000', zIndex: 1 }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)',
        width: 'calc(177.78vh)', minWidth: '100%', height: '100%', overflow: 'hidden' }}>
        <div ref={containerRef} style={{ width: '100%', height: '100%' }} />
      </div>

      {/* Tap overlay — toggle play/pause, blocks YouTube click events */}
      <div onClick={togglePlay} style={{ position: 'absolute', inset: 0, zIndex: 3, cursor: 'pointer' }} />

      {/* Thumbnail shown before ready */}
      {!ready && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 4, background: '#000' }}>
          <img
      ...
      {(!playing) && (
        <div onClick={togglePlay} style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 5, width: 68, height: 68, borderRadius: '50%', background: 'rgba(200,146,42,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 40px rgba(0,0,0,0.7)', cursor: 'pointer', pointerEvents: 'none' }}>
