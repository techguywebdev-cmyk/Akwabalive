...
export default function MomentsPage() {
  ...
  return (
    ...
    >
      {/* FIXED HEADER */}
      <div style={{ height: 80, flexShrink: 0, zIndex: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 18px', background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#000' }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 110, zIndex: 5, background: 'rgba(0,0,0,1)', pointerEvents: 'none' }} />
        ...
        {moment && (
          <iframe key={moment.id}
            ...
            style={{ position: 'absolute', top: '-80px', left: '50%', transform: 'translateX(-50%)', width: 'calc((100svh - 80px) * 16 / 9)', height: 'calc(100% + 160px)', minWidth: '100%', border: 'none', zIndex: 2 }}
        ...
        {lightbox && (
          <div
            ...
            onTouchStart={e => { touchY.current = e.touches[0].clientX; }}
            onTouchEnd={e => {
              const dx = touchY.current - e.changedTouches[0].clientX;
              if (Math.abs(dx) > 40) {
            ...
            style={{ position: 'absolute', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.92)', backdropFilter: 'blur(16px)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
