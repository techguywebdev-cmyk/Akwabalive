...
export async function parseEventLink(url: string): Promise<ParsedDraft> {
  ...
  const desc = (ogDesc || bodyText).slice(0, 600);
  const detected = detectArtistsFromText(title, desc, ocrText || '', venue);
  if (detected.artistSlugs.length) {
    notes.push(
      `Detected artists: ${detected.lineup.join(', ')}. Confirm lineup before publishing.`,
    );
    confidence += 0.05;
  }
