  ...
  return (
    <div style={{ marginBottom: 28 }}>
      ...
      <div style={{
        ...
        {items.map(ev => (
          <div key={ev.slug} style={{ width: 200, flexShrink: 0 }}>
            <EventCard
              event={ev}
              view="grid"
...
function EventCard({
  ...
  const saveBtn = onToggleSave ? (
    ...
    >
      {saveLoading ? <Loader2 size={14} className="spin" /> : <Star size={14} fill={saved ? C.gold : 'none'} />}
    </button>
  ) : null;
      ...
      <div style={{ position: 'relative' }}>
        <Link href={`/events/${event.slug}`} style={{
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4 }}>
            <p style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, color: C.gold, letterSpacing: '1px', textTransform: 'uppercase', margin: 0 }}>
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <div style={{ width: '100%', paddingTop: compact ? '66%' : '58%', position: 'relative', overflow: 'hidden', background: C.bg3 }}>
          <div style={{
          ...
          <div style={{ position: 'absolute', top: 10, left: 10, zIndex: 4, display: 'flex', gap: 6, flexWrap: 'wrap', maxWidth: '70%' }}>
            <span style={{
        ...
        <div style={{ padding: compact ? '12px 12px 14px' : '14px 16px 16px' }}>
          <p style={{ fontFamily: 'var(--font-inter,sans-serif)', fontSize: 10, color: C.c3, marginBottom: 5 }}>
            {city} · {event.venue}
          </p>
          <h3 style={{
            fontFamily: 'var(--font-cormorant,serif)', fontSize: compact ? 16 : 18, fontWeight: 400, color: C.cream,
