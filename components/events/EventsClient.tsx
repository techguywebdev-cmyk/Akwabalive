...
import {
  Search, MapPin, X, Grid2X2, List,
  Filter, ChevronDown, Calendar, Clock,
} from 'lucide-react';
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontFamily: 'var(--font-syne,sans-serif)', fontSize: 18, fontWeight: 700, color: C.cream }}>
            Filter Events
          </span>
...
function EventCard({ event, distanceKm, listMode = false }: {
  ...
  if (listMode) {
    return (
      <Link href={`/events/${event.slug}`} style={{ display: 'flex', borderBottom: `1px solid ${C.bd}`, textDecoration: 'none', transition: 'background 200ms' }}
        className="hover:bg-[rgba(245,236,215,0.03)]">
        <div style={{ position: 'relative', width: 160, height: 130, flexShrink: 0, overflow: 'hidden', backgroundImage: `url(${event.image})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'brightness(0.75)' }}>
        </div>
        <div style={{ flex: 1, padding: '14px 16px', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 10, marginTop: 8, borderTop: `1px solid ${C.bd}` }}>
            <span style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 14, color: C.cream }}>
              {event.price === 0 ? <span style={{ color: '#4ade80', fontSize: 9, fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase' }}>Free</span> : `₵${event.price}`}
            </span>
  ...
  return (
    <Link
      ...
      style={{ display: 'block', position: 'relative', overflow: 'hidden', borderRadius: 14, border: `1px solid ${C.bd}`, background: C.bg2, textDecoration: 'none', transition: 'transform 320ms cubic-bezier(.22,1,.36,1), box-shadow 320ms, border-color 320ms' }}
      className="group hover:-translate-y-1 hover:shadow-[0_20px_48px_rgba(0,0,0,0.55)] hover:border-[rgba(200,146,42,0.28)]"
    >
      {event.hot && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(to right,${C.gold},rgba(200,146,42,0.1))`, zIndex: 6 }} />}
      ...
      <div style={{ position: 'relative', width: '100%', paddingTop: '58%', overflow: 'hidden', background: C.bg3 }}>
        ...
        }} className="group-hover:scale-[1.07] group-hover:brightness-[0.4]" />
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 2, background: `linear-gradient(to right,${C.gold},rgba(200,146,42,0.1))`, zIndex: 5 }} className="origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />
    </Link>
  );
}
...
function SideRow({ label, count, active, onClick }: {
  ...
  return (
    <button onClick={onClick} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 8px', cursor: 'pointer', background: active ? 'rgba(200,146,42,0.07)' : 'transparent', transition: 'background 150ms', border: 'none', textAlign: 'left' }}
      className="hover:bg-[rgba(245,236,215,0.03)]">
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ width: 5, height: 5, borderRadius: '50%', background: active ? C.gold : C.bd, flexShrink: 0, transition: 'background 180ms' }} />
...
export default function EventsClient({ events }: { events: GhanaEvent[] }) {
  ...
  /* FILTER SHEET CONTENT — reused on mobile */
  const filterPanelContent = (
    <>
      ...
      <SheetSection title="Sort by">
        {(['upcoming', 'popular'] as const).map(s => (
          <Pill key={s} active={filters.sort === s} onClick={() => set('sort', s)}>
            {s === 'upcoming' ? '📅 Upcoming' : '🔥 Popular'}
          </Pill>
        ))}
        <Pill active={filters.sort === 'nearby'} onClick={() => { requestLoc(); set('sort', 'nearby'); }}>
          📍 Near Me
        </Pill>
      </SheetSection>
      ...
      {activeCount > 0 && (
        <div style={{ padding: '24px 20px 0' }}>
          <button onClick={() => { clear(); setSheet(false); }}
            style={{ width: '100%', fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, letterSpacing: '2px', textTransform: 'uppercase', color: '#CE1126', border: '1px solid rgba(206,17,38,0.2)', padding: '12px', background: 'transparent', cursor: 'pointer', borderRadius: 6, transition: 'all 200ms' }}>
            × Clear all filters
          </button>
        </div>
      )}
      ...
      <div style={{ padding: '16px 20px 0' }}>
        <button onClick={() => setSheet(false)}
          style={{ width: '100%', fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, letterSpacing: '2px', textTransform: 'uppercase', background: C.gold, color: '#000', padding: '14px', fontWeight: 700, cursor: 'pointer', border: 'none', borderRadius: 6 }}>
    ...
    <div style={{ background: C.bg, minHeight: '100vh' }}>
      ...
      <div style={{ position: 'sticky', top: 62, zIndex: 90, background: `${C.bg}F7`, backdropFilter: 'blur(20px)', borderBottom: `1px solid ${C.bd}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px' }}>
          ...
          {/* Filter button — opens bottom sheet on ALL screen sizes */}
          <button
            onClick={() => setSheet(true)}
            style={{
              display: 'flex', alignItems: 'center', gap: 8,
              fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 10, letterSpacing: '1.5px', textTransform: 'uppercase',
              border: `1px solid ${activeCount > 0 ? C.goldBd : C.bd}`,
              background: activeCount > 0 ? C.goldDim : C.bg2,
              color: activeCount > 0 ? C.gold : C.c3,
          ...
          <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
            {(['grid', 'list'] as const).map(m => (
              <button key={m} onClick={() => setView(m)}
                style={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${view === m ? C.goldBd : C.bd}`, background: view === m ? C.goldDim : 'transparent', color: view === m ? C.gold : C.c3, borderRadius: 6, cursor: 'pointer', transition: 'all 180ms' }}>
        ...
        {activeCount > 0 && (
          <div style={{ display: 'flex', gap: 6, padding: '0 16px 10px', overflowX: 'auto', scrollbarWidth: 'none' }}>
            ...
            <button onClick={clear} style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 9, letterSpacing: '1px', textTransform: 'uppercase', color: '#CE1126', background: 'transparent', border: 'none', cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap' }}>
              Clear all
            </button>
          </div>
        )}
      ...
      <div style={{ display: 'flex' }}>
        ...
        <aside style={{ display: 'none' }} className="lg:block lg:w-[240px] lg:flex-shrink-0 lg:sticky lg:top-[calc(62px+57px)] lg:self-start lg:max-h-[calc(100vh-130px)] lg:overflow-y-auto lg:border-r" style2={{ borderColor: C.bd, padding: '20px 16px' }}>
          <style>{`.lg-aside { display: block !important; }`}</style>
          <div className="hidden lg:block lg:w-[240px] flex-shrink-0 sticky self-start overflow-y-auto border-r"
            style={{ top: 'calc(62px + 57px)', maxHeight: 'calc(100vh - 130px)', borderColor: C.bd, padding: '20px 16px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, paddingBottom: 80 }}>
          ...
          <div style={{
            ...
            background: locStatus === 'found' ? 'rgba(45,106,79,0.1)' : C.bg2,
            borderBottom: `1px solid ${locStatus === 'found' ? 'rgba(45,106,79,0.2)' : C.bd}`,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{
                width: 7, height: 7, borderRadius: '50%', flexShrink: 0,
                background: locStatus === 'found' ? C.green : locStatus === 'error' ? C.red : C.c3,
                boxShadow: locStatus === 'found' ? `0 0 10px ${C.green}` : 'none',
                animation: locStatus === 'found' ? 'blink 2s ease-in-out infinite' : 'none',
              }} />
              ...
              {locStatus === 'found' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <select value={radius} onChange={e => setRadius(Number(e.target.value))}
                    style={{ background: 'transparent', border: `1px solid ${C.bd}`, color: C.c2, fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, padding: '3px 6px', outline: 'none' }}>
            ...
            {locStatus !== 'found' && (
              <button onClick={requestLoc}
                style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 7.5, letterSpacing: '2px', textTransform: 'uppercase', color: C.gold, border: `1px solid ${C.goldBd}`, padding: '7px 14px', borderRadius: 6, background: 'transparent', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
          ...
          {locStatus === 'found' && nearby.length > 0 && !nearbyDismissed && (
            <div style={{ borderTop: `2px solid rgba(45,106,79,0.2)`, background: 'linear-gradient(to bottom, rgba(45,106,79,0.07), transparent)', padding: '20px 16px 24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
                ...
                <button onClick={() => setNearbyDismissed(true)}
                  style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 7, letterSpacing: '2px', textTransform: 'uppercase', color: C.c3, border: `1px solid ${C.bd}`, padding: '5px 10px', background: 'transparent', cursor: 'pointer', borderRadius: 4 }}>
                  <X size={9} /> Hide
                </button>
              </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 14px', borderBottom: `1px solid ${C.bd}`, flexWrap: 'wrap', gap: 8 }}>
            ...
            <span style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 9, letterSpacing: '1px', color: C.c3 }}>
              <strong style={{ color: C.c2 }}>{visible.length}</strong> / <strong style={{ color: C.c2 }}>{filtered.length}</strong> events
            </span>
          </div>
          ...
          <div style={{
            ...
            {visible.length === 0 ? (
              ...
              visible.map(ev => (
                <EventCard
                  key={ev.id} event={ev} listMode={view === 'list'}
          ...
          {hasMore && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '8px 16px 32px' }}>
              ...
              <button onClick={() => setPage(p => p + BATCH)}
                style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8.5, letterSpacing: '2.5px', textTransform: 'uppercase', color: C.c3, border: `1px solid ${C.bd}`, padding: '13px 36px', borderRadius: 6, background: 'transparent', cursor: 'pointer', transition: 'all 220ms', display: 'flex', alignItems: 'center', gap: 8 }}
