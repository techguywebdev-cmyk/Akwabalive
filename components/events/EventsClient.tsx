...
function EventCard({
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4 }}>
          <p style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, color: C.gold, letterSpacing: '1px', textTransform: 'uppercase' }}>
            ...
            {distKm != null && distKm < 900 && (
              <span style={{ color: C.green, marginLeft: 8 }}>· {formatDistance(distKm)}</span>
            )}
          </p>
          <p style={{
            fontFamily: 'var(--font-cormorant,serif)', fontSize: 18, color: C.cream, lineHeight: 1.15,
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          }}>
            {event.title}
          </p>
          <p style={{ fontFamily: 'var(--font-inter,sans-serif)', fontSize: 11, color: C.c3 }}>
            {event.dateLabel} · {event.venue}
          </p>
          <p style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 11, color: event.price === 0 ? C.green : C.cream }}>
            {price}
          </p>
        </div>
      </Link>
  ...
  return (
    <Link href={`/events/${event.slug}`} style={{
      ...
      <div style={{ width: '100%', paddingTop: '62%', position: 'relative', overflow: 'hidden', background: C.bg3 }}>
        <div style={{
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        <p style={{ fontFamily: 'var(--font-inter,sans-serif)', fontSize: 10, color: C.c3, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
          <MapPin size={9} style={{ opacity: 0.5 }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
          <span style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 10, color: C.c2 }}>
            {event.dateLabel} · {event.time}
          </span>
          <span style={{
            fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 10, fontWeight: 700,
            color: event.price === 0 ? C.green : C.bg,
            background: event.price === 0 ? 'transparent' : C.gold,
            border: event.price === 0 ? `1px solid ${C.green}` : 'none',
            padding: '6px 10px', borderRadius: 6,
          }}>
            {event.price === 0 ? 'FREE' : 'GET TICKETS'}
          </span>
        </div>
      </div>
    </Link>
