  ...
  return (
    <>
      {open && (
        <div
          onClick={onClose}
          style={{ position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(13,11,8,0.85)', backdropFilter: 'blur(6px)' }}
        />
      )}
      <div style={{
...
function EventCard({ event, distanceKm, listMode = false }: {
  ...
  if (listMode) {
    return (
      <Link href={`/events/${event.slug}`} style={{ display: 'flex', borderBottom: `1px solid ${C.bd}`, textDecoration: 'none' }} className="hover:bg-[rgba(245,236,215,0.03)]">
        <div style={{ position: 'relative', width: 160, height: 130, flexShrink: 0, overflow: 'hidden' }}>
          <Image src={event.image} alt={event.title} fill sizes="160px" style={{ objectFit: 'cover', filter: 'brightness(0.75)' }} />
        </div>
  ...
  return (
    <Link
      ...
      onMouseEnter={e => { e.currentTarget.style.boxShadow = `0 20px 48px rgba(0,0,0,0.55)`; e.currentTarget.style.borderColor = C.goldBd; }}
      onMouseLeave={e => { e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.borderColor = C.bd; }}
    >
      ...
      <div style={{ position: 'relative', width: '100%', paddingTop: '58%', overflow: 'hidden', background: C.bg3 }}>
        <Image src={event.image} alt={event.title} fill sizes="(max-width:640px) 50vw,400px"
          style={{ objectFit: 'cover', filter: 'brightness(0.78) saturate(0.85)' }}
          className="transition-transform duration-700 group-hover:scale-[1.07] group-hover:brightness-[0.4]"
        />
      ...
      <div style={{ padding: '16px 18px 18px' }}>
        ...
        <h3 style={{ fontFamily: 'var(--font-cormorant,serif)', fontSize: 19, fontWeight: 400, color: C.cream, lineHeight: 1.18, marginBottom: 8, letterSpacing: '-0.2px' }} className="group-hover:text-white transition-colors">
          {event.title}
        </h3>
...
function SideRow({ label, count, active, onClick }: { label: string; count: number; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 8px', cursor: 'pointer', background: active ? 'rgba(200,146,42,0.07)' : 'transparent', border: 'none', textAlign: 'left', transition: 'background 150ms' }}
    ...
    <>
      ...
      {activeCount > 0 && (
        <div style={{ padding: '24px 20px 0' }}>
          <button onClick={() => { clear(); setSheet(false); }}
            style={{ width: '100%', fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, letterSpacing: '2px', textTransform: 'uppercase', color: C.red, border: `1px solid rgba(206,17,38,0.2)`, padding: 12, background: 'transparent', cursor: 'pointer', borderRadius: 6 }}>
            × Clear all filters
          </button>
        </div>
      )}
      <div style={{ padding: '16px 20px 0' }}>
        <button onClick={() => setSheet(false)}
          style={{ width: '100%', fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8, letterSpacing: '2px', textTransform: 'uppercase', background: C.gold, color: '#000', padding: 14, fontWeight: 700, cursor: 'pointer', border: 'none', borderRadius: 6 }}>
      ...
      <div style={{ position: 'sticky', top: 62, zIndex: 90, background: `${C.bg}F7`, backdropFilter: 'blur(20px)', borderBottom: `1px solid ${C.bd}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px' }}>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 10, letterSpacing: '1.5px', textTransform: 'uppercase', border: `1px solid ${activeCount > 0 ? C.goldBd : C.bd}`, background: activeCount > 0 ? C.goldDim : C.bg2, color: activeCount > 0 ? C.gold : C.c3, padding: '10px 16px', borderRadius: 8, cursor: 'pointer', flexShrink: 0, transition: 'all 180ms' }}
          >
            <Filter size={13} />
        ...
        {activeCount > 0 && (
          <div style={{ display: 'flex', gap: 6, padding: '0 16px 10px', overflowX: 'auto', scrollbarWidth: 'none' }}>
            {filters.city !== 'all' && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 9, letterSpacing: '1px', textTransform: 'uppercase', background: C.goldDim, color: C.gold, border: `1px solid ${C.goldBd}`, padding: '4px 10px', borderRadius: 999, flexShrink: 0 }}>
          ...
          {hasMore && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '8px 16px 32px' }}>
              ...
              <button onClick={() => setPage(p => p + BATCH)}
                style={{ fontFamily: 'var(--font-dm-mono,monospace)', fontSize: 8.5, letterSpacing: '2.5px', textTransform: 'uppercase', color: C.c3, border: `1px solid ${C.bd}`, padding: '13px 36px', borderRadius: 6, background: 'transparent', cursor: 'pointer', transition: 'all 220ms' }}
