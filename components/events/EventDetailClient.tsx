...
import {
  ...
  Share2, Heart, CheckCircle, Ticket, Star, Loader2, ExternalLink, Play, Volume2, VolumeX,
} from 'lucide-react';
...
export default function EventDetailClient({ event, related, seriesMomentId }: { event: GhanaEvent; related: GhanaEvent[]; seriesMomentId?: string | null }) {
  ...
  useEffect(() => {
    if (!imgReady || !(ytId || isMp4)) return;
    const t = window.setTimeout(() => setShowPromoVideo(true), 1400);
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', paddingTop: 62 }}>
      ...
      <div style={{ position: 'relative', width: '100%', height: 'min(65vw, 520px)', minHeight: 300, overflow: 'hidden', background: C.bg3 }}>
        ...
        {/* preload detect */}
        <img src={event.image} alt="" onLoad={() => setImgReady(true)} style={{ position: 'absolute', width: 1, height: 1, opacity: 0, pointerEvents: 'none' }} />
        {/* Shorts are 9:16 — scale to cover the wide hero frame */}
        {showPromoVideo && ytId && (
          <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', zIndex: 0 }}>
            <iframe
              key={promoMuted ? 'm' : 'u'}
              title="Announcement"
              src={`https://www.youtube.com/embed/${ytId}?autoplay=1&mute=${promoMuted ? 1 : 0}&controls=0&loop=1&playlist=${ytId}&playsinline=1&rel=0&modestbranding=1`}
              ...
              style={{
                ...
                /* scale up so vertical short fills horizontal frame */
        ...
        <div style={{ position: 'absolute', top: 16, right: 16, zIndex: 12, display: 'flex', gap: 8 }}>
          {showPromoVideo && (ytId || isMp4) && (
            <button
              type="button"
              onClick={() => setPromoMuted(m => !m)}
              aria-label={promoMuted ? 'Unmute' : 'Mute'}
              style={{
