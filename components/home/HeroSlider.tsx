...
const FLYER_OUTRO_MS = 2600;
/** Fallback if YouTube end event never fires */
...
/** Avoid conflicting Window.YT ambient types — use loose access */
type YTWin = Window & {
...
export default function HeroSlider({ initialSlides }: { initialSlides?: HeroSlide[] }) {
  ...
  /** Only reveal player after YT reports PLAYING — hides seek chrome flash */
  ...
  // Any real user gesture on the page unlocks sound for the YT player
  useEffect(() => {
    const unlock = () => {
    ...
    // Capture phase so we catch taps even on overlays
  ...
  // Create YT player when entering video phase
  useEffect(() => {
    ...
    (async () => {
      ...
      playerRef.current = new w.YT!.Player(host, {
        ...
        playerVars: {
          autoplay: 1,
          // Start muted so autoplay is allowed; unmute after gesture (or immediately if already unlocked)
        ...
        events: {
          onReady: (e: any) => {
            ...
            videoMaxTimer.current = setTimeout(() => {
              onVideoEndedRef.current();
            }, VIDEO_MAX_MS);
          },
          onStateChange: (e: any) => {
            // 1 = PLAYING — safe to show (no seek UI flash)
            if (e.data === 1) {
  ...
  // When sound unlocks mid-play, unmute live player
  useEffect(() => {
    if (!soundOn) return;
    try {
  ...
  return (
    ...
    >
        ...
        return (
          ...
          >
            <div style={{
              ...
              // Keep flyer until player is actually playing
      ...
      {/* Single YT mount — only visible once PLAYING */}
      ...
      {hasPromo && phase === 'video' && isMp4 && slide.promoVideoUrl && !ytId && (
        <video
          ...
          playsInline
          muted={!soundOn}
          onPlaying={() => setPlayerVisible(true)}
