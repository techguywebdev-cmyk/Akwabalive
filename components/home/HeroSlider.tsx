...
function ytSrc(id: string, sound: boolean) {
  return `https://www.youtube.com/embed/${id}?autoplay=1&mute=${sound ? 0 : 1}&controls=0&loop=1&playlist=${id}&playsinline=1&rel=0&modestbranding=1&iv_load_policy=3&fs=0&disablekb=1&cc_load_policy=0`;
}
...
export default function HeroSlider({ initialSlides }: { initialSlides?: HeroSlide[] }) {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setShowVideo(true), 800);
    return () => window.clearTimeout(t);
  }, [cur, ytId, isMp4]);

  // First tap/scroll unlocks audio (no UI icon — browsers block unmuted autoplay)
  useEffect(() => {
    if (!showVideo) return;
          ...
          >
            ...
            {playThis && sYt && (
              <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
                ...
                />
                <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 64, background: 'linear-gradient(to top, rgba(13,11,8,0.95), transparent)', pointerEvents: 'none' }} />
                <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: 48, background: 'linear-gradient(to bottom, rgba(13,11,8,0.45), transparent)', pointerEvents: 'none' }} />
              </div>
            )}
