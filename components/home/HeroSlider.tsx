...
export default function HeroSlider({ initialSlides }: { initialSlides?: HeroSlide[] }) {
  ...
  // Image first, then promo video on current slide
  useEffect(() => {
    setShowVideo(false);
    setMuted(true);
    if (!(ytId || isMp4)) return;
    const t = window.setTimeout(() => setShowVideo(true), 1200);
  ...
  return (
    ...
    >
            ...
            {playThis && sYt && (
              <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
                <iframe
                  key={`${sYt}-${muted ? 'm' : 'u'}`}
                  title="Promo"
                  src={`https://www.youtube.com/embed/${sYt}?autoplay=1&mute=${muted ? 1 : 0}&controls=0&loop=1&playlist=${sYt}&playsinline=1&rel=0&modestbranding=1`}
      ...
      {/* Sound toggle when promo is playing */}
      {showVideo && (ytId || isMp4) && (
        <button
          type="button"
