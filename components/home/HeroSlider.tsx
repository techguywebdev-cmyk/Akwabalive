...
/** Absolute max if ENDED never fires */
...
export default function HeroSlider({ initialSlides }: { initialSlides?: HeroSlide[] }) {
  ...
  /** Suppress synthetic click after touch-nav (prevents jump to /events) */
  ...
  const beginFlyerOut = useCallback(() => {
    // Ignore bogus ENDED if video barely started
    ...
    setPhase('flyer-out');
    setProg(90);
    phaseTimer.current = setTimeout(() => goTo(cur + 1), FLYER_OUTRO_MS);
  }, [cur, goTo]);
        ...
        events: {
          ...
          onStateChange: (e: any) => {
            ...
            if (e.data === 1) {
              ...
              // Prefer real duration so we never cut the short short
              try {
                const dur = Number(e.target.getDuration?.()) || 0;
                if (videoMaxTimer.current) clearTimeout(videoMaxTimer.current);
                const hold = dur > 3 ? Math.min(dur * 1000 + 800, VIDEO_MAX_MS) : VIDEO_MAX_MS;
            ...
            // ENDED — only if played long enough
            if (e.data === 0) {
              onVideoEndedRef.current();
            }
          },
          onError: () => {
            // Don't skip instantly — show flyer outro
            if (playStartedAtRef.current && Date.now() - playStartedAtRef.current > 2000) {
              onVideoEndedRef.current();
            } else {
              setTimeout(() => onVideoEndedRef.current(), 5000);
            }
          },
        },
      });
  ...
  return (
    <section
      ...
      // Block the ghost click that follows touch-nav (often hits a Link under the hero)
      onClickCapture={(e) => {
    ...
    >
      ...
      {hasPromo && phase === 'video' && ytId && (
        ...
        >
          ...
          />
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 96, background: 'linear-gradient(to top, #0D0B08 30%, transparent)', pointerEvents: 'none' }} />
      ...
      {/* Explicit left / right hit zones — do not use full-section click (ghost clicks → /events) */}
      <button
        type="button"
        aria-label="Previous slide"
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); navPrev(); }}
        style={{
          position: 'absolute',
      ...
      <button
        ...
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); navNext(); }}
        style={{
          position: 'absolute',
      ...
      <div style={{
        ...
        {slides.map((_, i) => (
          <button
            key={i}
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); armClickSuppress(); goTo(i); }}
